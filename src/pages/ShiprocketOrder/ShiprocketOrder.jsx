...
export default function ShiprocketOrder() {
    ...
    let[toDate , setToDate] = useState(today)
    ...
    const getOrders = async(page , items)=>{
      setOrders(false)
      try{
        let result = await apiObj.getAllShiprocketOrders( fromDate ,toDate ,  page , items, "All",headers)
        console.log(result)
    ...
    try{
        let result = await apiObj.createShipment(data , headers)
  ...
  const cancelOrder = async(orderId)=>{
      let data = {
        ...
        try{
          let result = await apiObj.cancelOrder(data ,headers)
          getOrders()
          setDeleteBox(false)
      notifySuccess("Order Cancelled Successfully ")
          console.log(result)
        ...
        <div className="pt-5 max-w-[100vw] overflow-hidden ">
          ...
          <table className="w-full min-w-[950px]   border">
            ...
            <tbody>
              ...
              <tr className={`border p-2 hover:bg-white ${ order && order.shiprocketDetails &&  order.shiprocketDetails.status == filter || filter == "ALL" ? '' : 'hidden'} `} key={index}>
                <td className="text-center  border">{order.shiprocketDetails?.id || order?.order_id}</td>
                <td className="text-center  border">{order?.channel_order_id}</td>
                
                <td className="text-center  border">{order.shiprocketDetails?.customer_name || order?.userId?.name}</td>
                <td className="text-center w-[] border">{order?.total?.toFixed(2)}
                  <br />
                  <p className={` flex justify-center gap-2 ${order.shiprocketDetails?.cod ? "text-[red]" : "text-[green]"} text-[13px]`}>

                  {order?.shiprocketDetails?.cod ? "COD" : "Prepaid"} 
                   <span>
                ...
                <td className="text-center w-[] border">{order.createdAt.slice(0,10)}</td>
                <td className={`text-center w-[] border ${order?.shiprocketDetails?.status == "CANCELED" || order?.shiprocketDetails?.status == "CANCELLATION REQUESTED" || order?.shiprocketDetails?.status == 'INVOICED' || order.status == "CANCELED" ? 'text-[red]' : 'text-[green]'} `}>{order?.shiprocketDetails?.status == 'INVOICED' ? "CANCELED" : order?.shiprocketDetails?.status  || order?.status}</td>
                ...
                <td className="text-center w-[] border">
                 <p className="flex gap-3 w-full items-center justify-center relative">
                  {/* <IoEyeOutline className="text-[#696cff] text-[20px] cursor-pointer" onClick={()=>navigate(`/view-order/${order.shiprocketDetails.id}`)}/> */}
                    <button className={`${ order?.shiprocketDetails?.status == "NEW" ? "" : "hidden"} px-3 py-1 bg-[#696cff] text-white`} onClick={()=>createShipment(order?.shipment_id , order?.courierCompanyId)}>Ship Now</button>
                    <button className={`${ order?.shiprocketDetails?.status == "READY TO SHIP"  ? "" : "hidden"} px-3 py-1 bg-[#696cff] text-white`} onClick={()=>reqPic()}>Request for Pic</button>
                  {/* <MdOutlineModeEdit className=" text-[20px] cursor-pointer" 
                  onClick={()=>navigate(`/edit-product/${order._id}`)}
                  /> */}
                  <IoIosMore className="text-[30px] text-[#696cff] cursor-pointer" onClick={()=>{
                    setCurrentOrder(order)
                    setShowMore(true)
                  }}/>
                  {/* more menu */}
                    <div className={`${currentOrder && currentOrder == order && showMore ? "fixed" : "hidden"} h-full w-full  shadow-lg top-0 left-0 z-20 py-2`} onClick={()=>{
                      setCurrentOrder(false)
                      setShowMore(false)
                    }}>

                    </div>
                    <div className={`${currentOrder && currentOrder == order && showMore ? "absolute" : "hidden"}  w-[150px] bg-white shadow-lg top-7 right-2 z-20 py-2`}>
                        <p className="hover:bg-[#c5c5eb] hover:text-[#696cff] cursor-pointer py-1"  onClick={()=>{
                      downloadInvoice(order.order_id)
                      setShowMore(false)
                  }}>Download Invoice</p>
                        <p className="hover:bg-[#c5c5eb] hover:text-[#696cff] cursor-pointer py-1"  onClick={()=>{
                      downloadLabel(order.shipment_id)
                      setShowMore(false)
                  }}>Download Label</p>
                        
                        <p className={`hover:bg-[#c5c5eb] hover:text-[#696cff] cursor-pointer ${currentOrder && currentOrder?.shiprocketDetails?.status == "NEW" ? "" : "hidden"} py-1 `} onClick={()=>{
                          setCurrentOrder(order)
                          setEditOrder(true)
                          setHeight(order?.shiprocketDetails?.shipments[0]?.height ? order?.shiprocketDetails?.shipments[0]?.height : order?.shiprocketDetails?.others?.update_request_log?.request_data && order?.shiprocketDetails?.others?.update_request_log?.request_data?.height ? order?.shiprocketDetails?.others?.update_request_log?.request_data?.height : 0)


                          setBreadth(order?.shiprocketDetails?.shipments[0]?.breadth ? order?.shiprocketDetails?.shipments[0]?.breadth : order?.shiprocketDetails?.others?.update_request_log?.request_data && order?.shiprocketDetails?.others?.update_request_log?.request_data?.breadth ? order?.shiprocketDetails?.others?.update_request_log?.request_data?.breadth : 0)


                          setLength(order?.shiprocketDetails?.shipments[0]?.length ? order?.shiprocketDetails?.shipments[0]?.length : order?.shiprocketDetails?.others?.update_request_log?.request_data && order?.shiprocketDetails?.others?.update_request_log?.request_data?.length ? order?.shiprocketDetails?.others?.update_request_log?.request_data.length : 0)


                          setWeight(order?.shiprocketDetails?.shipments[0]?.weight ? order?.shiprocketDetails?.shipments[0]?.weight : order?.shiprocketDetails?.others?.update_request_log?.request_data && order?.shiprocketDetails?.others?.update_request_log?.request_data?.weight ? order?.shiprocketDetails?.others?.update_request_log?.request_data?.weight : 0)
                        }}>Edit Order</p>
                        <p className="hover:bg-[#c5c5eb] hover:text-[#696cff] cursor-pointer py-1" onClick={()=>navigate(`/view-order/${order?.shiprocketDetails?.id || order?.order_id}`)}>View Order</p>
                        <p className={`hover:bg-[#c5c5eb]  cursor-pointer ${currentOrder && currentOrder.shiprocketDetails?.status == "NEW" || currentOrder && currentOrder?.shiprocketDetails?.status == "READY TO SHIP" ? "" : "hidden"} py-1 text-[red] `} onClick={()=>{
                          setCurrentOrder(order)
                          setDeleteBox(true)
                          setShowMore(false)
                        }}>Cancel Order</p>
                  ...
                  {/* <RiFileDownloadLine className="text-[green] text-[20px] cursor-pointer" alt="Download Invoice" onClick={()=>{
                      downloadInvoice(order.order_id)
                  }} /> */}
    ...
    <p className="bg-[#f5f5f9] p-2 text-[red] text-center font-bold fixed w-full  top-0 right-0 ">Cancel Order</p>
      ...
      <p className="flex justify-between px-4 pt-6">
        ...
        <button className="p-2 px-4 text-white bg-[#696cff] rounded-md" onClick={()=>{
            if(currentOrder){
                currentOrder.shiprocketDetails.status == "NEW" ? cancelOrder(currentOrder.order_id) : cancelShipment(currentOrder.shiprocketDetails.shipments[0].awb ,currentOrder.order_id)
            }
        }}>Sure</button>
      </p>
