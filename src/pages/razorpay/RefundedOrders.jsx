...
export default function RefundedOrders() {
    ...
    const getRazorpayReturnedPayments = async()=>{
      try{
        let result = await apiObj.getRazorpayRefundPayment(headers)
        if(result.data.refunds && result.data.refunds.length > 0 ){
          setOrders(result.data.refunds)
        }else{
          setOrders('no data')
        }
        console.log(result)
        ...
        <div className=" bg-white shadow-md rounded-sm  mb-4">
            ...
            <div>
                ...
                <div className="flex justify-between flex-wrap">
                    <div className={`sm:w-[33%] w-full p-3  ${
        ...
        <div className="pt-5 max-w-[100vw] overflow-hidden ">
          ...
          <table className="w-full min-w-[950px]  border">
            ...
            <tbody>
              ...
              <tr className={`border p-2 hover:bg-white text-[14px] cursor-pointer `} key={index} 
            //   onClick={()=>navigate(`/view-razorpay-payment/${order.id}`)}
              >
